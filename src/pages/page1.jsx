import React from 'react'
import {
    FormControl,
    InputGroup,
    Button,
    Image,
} from 'react-bootstrap'
import { Link } from 'react-router-dom'

class Page1 extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            visibility: false,
            check: false

        }
    }

    render() {
        const { visibility } = this.state
        return (
            <div style={{ display: 'flex', flexDirection: 'column' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', margin:'3vw' }}>
                    <Image style={{ height: '10vh', width: '20vw', marginLeft:'2vw' }} src={"https://raw.githubusercontent.com/nafis-tech/ID_Core_FE/master/src/images/logo.png"}></Image>
                    <div style={{ display: 'flex', margingLeft:'5vw', fontSize:'10px',}}>
                        <Button  style={{fontSize:'13px', marginRight:'2vw', borderRadius:'30px', height:'6vh'}} variant="outline-warning" >Cara menggunakan Nova Frelancer?</Button>
                        <Button  style={{fontSize:'13px', marginRight:'5vw', borderRadius:'30px', height:'6vh'}} variant="outline-warning">Daftar sebagai Frelancer</Button>
                    </div>
                </div>
                <div style={{ display: 'flex', marginTop:'8vh', marginBottom:'8vh' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', flexBasis: '50%', margin: ' 0 3vw 0 10vw'}}>
                        <p>Welcome back</p>
                        <h3 style={{ color: '#FFC107', margin: ' 2vh 0 3vh 0' }}>Login to your account</h3>
                        <label>Email</label>
                        <InputGroup className="mb-3">
                           <FormControl
                                placeholder="Input Your Password"
                                ref="username"
                                style={{ color: '#012EA9' }}

                            />
                        </InputGroup>
                        <label>Password</label>
                        <InputGroup className="mb-3">
                            <FormControl
                                placeholder="Input Your Email"
                                type={visibility ? "text" : "password"}
                                ref="password"
                                style={{ color: '#012EA9' }}
                            />
                        </InputGroup>
                        <div style={{ display: 'flex', justifyContent: 'space-between', margin: '3vh' }}>
                            <label style={{ fontSize: '13px' }}>
                                <input type="radio" value="option1" style={{ marginRight: '1vw' }} />
                                Remember me
                            </label>
                            <Link style={{ color: '#03256C', fontSize: '13px', fontWeight: 'bold' }} to="/">Forgot Password?</Link>
                        </div>
                        <Button href="/page3" style={styles.button}>
                            Login Now
                        </Button>
                        <Button style={styles.button2}>
                            <Image style={{ width: '2vw' }} src={"data:image/png;base64,"}></Image>
                            <p style={{ marginLeft:'1vw'}} >Or Sign-in with google</p>
                        </Button>
                        <h6 style={{ marginTop: '7vh', fontSize: '13px', textAlign: 'center' }}>Don't have an account? <Link style={{ color: '#03256C', fontWeight: 'bold' }} to="/">Join free today</Link></h6>
                    </div>
                    <div style={{ display: 'flex', flexBasis: '50%', backgroundColor: 'orange' }}>
                        <Image src={"https://raw.githubusercontent.com/nafis-tech/ID_Core_FE/master/src/images/Screenshot%20(1542).png"}
                            style={{ width: '58vw', height: '110vh',marginTop:'0' }}></Image>
                    </div>
                </div>
            </div>
        )
    }
}

const styles = {
    button: {
        backgroundColor: '#FFC107',
        border: 'none',
        margin: '3vh 0 0 0',
        padding: '2vh 0 2vh 0'
    },
    button2: {
        backgroundColor: 'white',
        color: 'black',
        // border: '2px solid #03256C',
        border: 'none',
        margin: '4vh 0 0 0',
        display: 'flex',
        paddingLeft: '5vw',
        paddingTop: '2vh'
    },
    p: {
        fontSize: '13px'
    }
}

export default Page1;